...
def process_eeg_domain(path=BASE_DIR / 'dataset/eeg/data/domain_feature/train_data1.mat', fold=0, saving_name='dataname'):
    ...
    x = dict()
    x[0] = MinMaxScaler((0, 1)).fit_transform(data['X'][0].T).astype(np.float32)
    x[1] = MinMaxScaler((0, 1)).fit_transform(data['X'][1].T).astype(np.float32)
    x[2] = MinMaxScaler((0, 1)).fit_transform(data['X'][2].T).astype(np.float32)
